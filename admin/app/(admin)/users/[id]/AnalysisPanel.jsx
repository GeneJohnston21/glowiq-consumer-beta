...
export default function AnalysisPanel({ entry, SEV_COLOR, SEV_ORDER }) {
  ...
  return (
    <div style={{ background:'white', border:`1px solid ${BR}`, borderRadius:12, overflow:'hidden' }}>
      ...
      <div onClick={() => setOpen(o => !o)}
        style={{ padding:'16px 24px', cursor:'pointer', display:'flex', alignItems:'center',
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
            <span style={{ fontFamily:FS, fontSize:13, fontWeight:500, color:TX }}>{entry.skinType || 'Unknown skin type'}</span>
        ...
        <div style={{ borderTop:`1px solid ${BR}` }}>
          <div style={{ display:'grid', gridTemplateColumns:'300px 1fr', gap:0 }}>
            ...
            <div style={{ padding:'20px 24px', borderRight:`1px solid ${BR}` }}>
              {entry.thumb && (
                <img src={entry.thumb} alt="Full scan"
                  style={{ width:'100%', borderRadius:10, border:`1px solid ${BR}`, marginBottom:16, display:'block' }} />
              )}
                  ...
                  {sorted.map((c, i) => {
                    ...
                    return (
                      <div key={i} style={{ padding:'12px 14px', borderRadius:8,
                        background:sv.bg, border:`1px solid ${sv.br}` }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:c.description?6:0 }}>
                          ...
                          <span style={{ fontFamily:FS, fontSize:10, padding:'1px 7px', borderRadius:4,
                            background:'rgba(255,255,255,.6)', border:`1px solid ${sv.br}`, color:sv.tx }}>
                            {c.severity}
                          </span>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                    {entry.recommendations.map((r, i) => (
                      <div key={i} style={{ padding:'10px 14px', borderRadius:8,
                        ...
                        {r.category && (
                          <div style={{ fontFamily:FS, fontSize:11, color:MU, marginTop:2 }}>{r.category}</div>
                        )}
                      </div>
                    ))}
                  </div>
